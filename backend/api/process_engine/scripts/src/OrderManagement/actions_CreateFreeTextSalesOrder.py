...
def action():
    free_text = DocumentApi(document_id='SOD').get_document_dict()['FreeText']
    # print(free_text)
        ...
        messages=[
            {
                    ...
                    "materials": [{"Quantity": "12", "Unit": "pcs"}]
                    """
                },
                {
                    "role": "user",
    ...
    for material in json_response['materials']:
        ...
        id = id.replace(' ', '_')
        wrapped_data = str(DocumentMasterDataWrapper(document_id='SOD', lead_object='Material', master_data_id=id, data=material))
